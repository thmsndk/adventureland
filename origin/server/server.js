...
io.sockets.on("connection", function (socket) {
	...
	socket.on("disconnect", function () {
		socket.get("user", function (err, user) {
			for (var i = 0; i < players.length; i++) if (players[i].id == user.id) players.splice(i, 1);
		});
	});
});
...
function check_user(user, chr, pass, appid, socket) {
	...
	if (!data.map) data.map = "start";
	if (!data.character) {
		if (data.gender == "female") data.chtype = "deffemale";
		else data.chtype = "defmale";
	}
...
function handle_player(user, chr, pass, appid, socket) {
	...
	GAE.on("error", function (err) {
		console.log(err);
		setTimeout(function () {
			handle_player(user, chr, pass, appid, socket);
		}, 500);
	});
	...
	request.on("response", function (response) {
		...
		response.on("end", function () {
			check_user(user, chr, pass, appid, socket);
		});
	});
}
