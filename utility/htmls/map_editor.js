	...
	if (!already) {
		if (anim) {
			var interval = parseInt($(".interval").val()),
				delay = $(".delay").val(),
				zoffset = parseFloat($(".zoffsetval").val());
...
function place_area(num, x, y, lx, ly, already) {
	...
	if (tiling) {
		tilings++;
		var entity = new PIXI.extras.TilingSprite(texture_cache[name], xs * def[3], ys * nunv(def[4], def[3]));
		sprites++;
		entity.x = x;
		entity.y = y;
		map.addChild(entity);
...
function line_check(visited, type, current) {
	...
	if (type == "x") {
		...
		for (var i = 0; i < (map_data.x_lines || []).length; i++) {
			...
			if (
				current[0] == line[0] &&
				((current[1] <= line[1] && line[1] <= current[2]) || (current[1] <= line[2] && line[2] <= current[2]))
			) {
				visited.x[i] = true;
				paths += 1;
...
function map_click(event) {
	...
	var cx = (event.data.global.x - map.x) / scale,
		cy = (event.data.global.y - map.y) / scale;
	...
	if (mode == "alert") {
		...
		if (map_data.polygons)
			for (var z in map_data.polygons || {}) {
				var found = false,
					p = null,
					min_x = map_data.polygons[z][0][0],
					max_x = map_data.polygons[z][0][0],
					min_y = map_data.polygons[z][0][1],
					max_y = map_data.polygons[z][0][1];
				for (var i = 0; i < map_data.polygons[z].length; i++) {
					...
					if (point_distance(cx, cy, point[0], point[1]) < 6) found = true;
				}
				if (found)
					return show_alert(
						"Polygon: [" +
	...
	if (mode == "alert" && !c_pressed && !x_pressed && !z_pressed) show_alert("x: " + cx + " y: " + cy);
		...
		// line coloring, exhaustive, to make sure lines are connected
		var done = false,
			visited;
		for (var i = 0; i < (map_data.x_lines || []).length; i++) {
			var current = map_data.x_lines[i];
		...
		var to_delete = [];
		for (
			var i = map_data.placements.length - 1;
			i >= 0;
			i-- // descending + break so only the first one is deleted
		) {
		...
		for (var i = to_delete.length - 1; i >= 0; i--) map_data.placements.splice(to_delete[i], 1);
	} else if (x_pressed) {
		// singular-delete
		var to_delete = [];
		for (
		...
		for (var i = to_delete.length - 1; i >= 0; i--) map_data.lights.splice(to_delete[i], 1);
		...
		if (!done) {
			...
			for (var i = to_delete.length - 1; i >= 0; i--) map_data.nights.splice(to_delete[i], 1);
		}
		...
		if (!done) {
			...
			for (var i = to_delete.length - 1; i >= 0; i--) map_data.animations.splice(to_delete[i], 1);
		}
		...
		var rx = floor(cx / step_x) * step_x,
			ry = floor(cy / step_y) * step_y,
			t,
			rxl = rx,
			ryl = ry;
		if (abs(rx - polygon[polygon.length - 1][0]) < abs(ry - polygon[polygon.length - 1][1])) {
			if (ry != polygon[polygon.length - 1][1]) polygon[polygon.length] = [polygon[polygon.length - 1][0], ry];
		} else {
			if (rx != polygon[polygon.length - 1][0]) polygon[polygon.length] = [rx, polygon[polygon.length - 1][1]];
		}
		if (
...
function map_mmove(event) {
	...
	if (window.indicator)
		try {
			indicator.parent.removeChild(indicator), indicator.destroy({ children: true }), (indicator = null);
		} catch (e) {}
	if (mode == "alert") indicator = draw_crosshair(cx, cy, 3, 0xdb2a1b);
		...
		else indicator = draw_line(line_start[0], line_start[1], floor(cx / step_x) * step_x, line_start[1], 2, 0x00bb00);
	} else if (mode == "rectangle") {
		...
		if (
			abs(floor(cx / step_x) * step_x - polygon[polygon.length - 1][0]) <
			abs(floor(cy / step_y) * step_y - polygon[polygon.length - 1][1])
		)
			indicator = draw_line(
				polygon[polygon.length - 1][0],
				polygon[polygon.length - 1][1],
				polygon[polygon.length - 1][0],
				floor(cy / step_y) * step_y,
				2,
				0x00bb00,
			);
		else
...
function map2_mmove(event) {
	last_move = "map2";
	var cx = (event.data.global.x - ((window.tileset && window.tileset.x) || 0)) / tileset_scale,
		cy = (event.data.global.y - ((window.tileset && window.tileset.y) || 0)) / tileset_scale,
		size = 2;
	if (tileset_scale > 1) size = 1;
	if (window.indicator2)
		try {
			indicator2.parent.removeChild(indicator2), indicator2.destroy(), (indicator2 = null);
		} catch (e) {}
...
function tileset_zoom(num) {
	var mid_x = renderer2.width / 2.0 - tileset.x,
		mid_y = renderer2.height / 2.0 - tileset.y;
...
function startstop_entity() {
	if (cgroup) startstop_group();
	else if (anim) startstop_animation();
	else show_modal($("#entitymodal").html(), { wrap: false });
}
...
function startstop_polygon() {
	if (mode == "polygon") {
		...
		mode = "polygon";
		$(".idbuttonid").val(randomStr(5));
		$(".idbutton").css("display", "inline-block");
		polygon = [];
...
function register_rectangle(x, y) {
	if (r_start[0] != x || r_start[1] != y) {
		...
		map_data.rectangles[$(".idbuttonid").val()] = [
			min(x, r_start[0]),
			min(y, r_start[1]),
			max(x, r_start[0]),
			max(y, r_start[1]),
...
function load_tileset(name, file) {
	var new_mode = true;
	window.tileset_scale = 1;
	window.tileset_move = false;
	if (set == name) destroy_tileset();
	else {
		if (set) destroy_tileset();
		if (location.search.indexOf("old_mode") != -1) {
			renderer2 = new PIXI.CanvasRenderer(C[file].width, C[file].height, {
				antialias: false,
				transparent: false,
			...
			renderer2 = new PIXI.CanvasRenderer(
				min(parseInt($(window).width() / 2), max(600, min(C[file].width, width))),
				min(parseInt($(window).height() - 80), max(400, min(C[file].height, height))),
				{ antialias: false, transparent: false, resolution: window.devicePixelRatio, autoResize: true },
			);
			stage2 = new PIXI.Container();
		...
		if (set_cache[set])
			(tileset_scale = set_cache[set].scale),
				(entity.scale = new PIXI.Point(tileset_scale, tileset_scale)),
				(entity.x = set_cache[set].x),
				(entity.y = set_cache[set].y);
		...
		if (new_mode)
			$("#toprightcorner").append(
				"<div style='position: absolute; top: 10px; right: 110px' class='gamebutton tilesetmove' onclick='$(\".tilesetmove\").toggleClass(\"active\"); cache_set()'>M</div> <div style='position: absolute; top: 10px; right: 60px' class='gamebutton' onclick='tileset_zoom(-1)'>-</div> <div style='position: absolute; top: 10px; right: 10px' class='gamebutton' onclick='tileset_zoom(1)'>+</div>",
			);
...
function show_upload_modal() {
	...
	html +=
		'<form action="{{upload_url}}" method="post" enctype="multipart/form-data">"this_map": <input type="file" name="image"/><input type="hidden" name="iname" value="map" /><input type="hidden" name="key" value="{{name}}"><input type="submit" name="submit" value="Upload"/></form>';
	html +=
		'<form action="{{upload_url}}" method="post" enctype="multipart/form-data">"this_map_a": <input type="file" name="image"/><input type="hidden" name="iname" value="map_a" /><input type="hidden" name="key" value="{{name}}"><input type="submit" name="submit" value="Upload"/></form>';
	html += "</div>";
	if (set) destroy_tileset();
	set = "upload";
...
function rescale_map(nscale) {
	var ox = (map.x - round(width / 2)) / scale,
		oy = (map.y - round(height / 2)) / scale;
...
function redraw_map() {
	...
	if (map_data.groups) {
		for (var p = 0; p < map_data.groups.length; p++) {
			try {
				...
				console.log("Faulty group detected + deleted");
				deleted = true;
				to_delete.push(i);
			}
		}
	...
	if (lines && map_data.polygons) {
		for (var z in map_data.polygons) {
			...
			for (var i = 0; i < poly.length; i++) {
				...
				if (poly[i][0] < mx) (mx = poly[i][0]), (my = poly[i][1]);
				else if (poly[i][0] == mx) my = min(poly[i][1], my);
			}
			var fp = {
				fontFamily: "Pixel",
				fontSize: 16,
	...
	if (lines && map_data.animations)
		for (var p = 0; p < map_data.animations.length; p++) {
			var tile = map_data.animations[p],
				offset = tile[7];
			var def = map_data.tiles[tile[0]],
				w = def[3],
				h = nunv(def[4], def[3]),
			...
			map.addChild(
				draw_line(
					tile[1] / 2.0 + (nunv(tile[3], tile[1]) + w) / 2.0 - 4,
					nunv(tile[4], tile[2]) + h + offset,
					tile[1] / 2.0 + (nunv(tile[3], tile[1]) + w) / 2.0 + 4,
					nunv(tile[4], tile[2]) + h + offset,
					2,
					color,
	...
	if (lines && map_data.groups)
		// for the group indicator lines
		for (var p = 0; p < map_data.groups.length; p++) {
			if (!map_data.groups[p].length) continue;
			var max_y = -9999999999,
				max_x = -999999999,
				...
				rx,
				line,
				offset = map_data.groups[p][0][5] || 0;
			if (cgroup - 1 === p) map_data.groups[p][0][5] = offset = parseFloat($(".zoffsetval").val()) || 0;
			for (var i = 0; i < map_data.groups[p].length; i++) {
				var tile = map_data.groups[p][i],
					def = map_data.tiles[tile[0]];
				var w = def[3],
			...
			(pack.boundaries || [pack.boundary]).forEach(function (b) {
				...
				map.addChild(
					empty_rect(
						pack.boundary[0],
						pack.boundary[1],
						pack.boundary[2] - pack.boundary[0],
						pack.boundary[3] - pack.boundary[1],
						2,
						0xdd3d3f,
					),
				);
				map.addChild(
					add_monster({
						type: pack.type,
						x: (pack.boundary[2] + pack.boundary[0]) / 2,
						y: (pack.boundary[3] + pack.boundary[1]) / 2,
					}),
				);
			});
		});
